    ...
    <?php if( have_rows('type2_menu') ): ?>
      <?php while ( have_rows('type2_menu') ) : the_row(); ?>
        ...
        <?php if( get_row_layout() == 'link_image_block' ): ?>
          <!-- Block type image -->
          <a href="<?php the_sub_field('link_href');?>" class="main-menu__sub-block main-menu__image-block type2 col-4">
            <?php echo wp_get_attachment_image(get_sub_field('link_image'), 'menu-thumb2'); ?>
            <div class="image-bg"
                  style="
                        background-image:url(<?php $a = wp_get_attachment_image_src(get_sub_field('link_image'), 'full'); echo $a[0]; ?>);
                  "
            >
            </div>
